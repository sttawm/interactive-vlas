...
All instances serve the **same web page** (a live camera stream + instruction box + VLA·Env·Scene
selectors) from [`shared/webui.py`](shared/). The upstream reference repos (MolmoSpaces,
MolmoBot, MolmoAct2) are cloned under `molmo/`, which is gitignored — they are not vendored.
